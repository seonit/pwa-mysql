...
SELECT LPAD('111', 4, '0'), RPAD('1', 4, '0');
...
SELECT
	...
	,LTRIM('	트림	')
;
-- TRIM(방형 문자열1 FROM 문자열2)
-- 방향을 지정해서 문자열 2에서 문자열 1을 제거하여 반환
-- 방향은 LEADING(좌), BOTH(좌우), TRAILING(우) 지정 가능
-- 'Bearer 34j6gifdasdFEW12'
SELECT TRIM(LEADING 'Bearer ' FROM 'Bearer 34j6gifdasdFEW12');
